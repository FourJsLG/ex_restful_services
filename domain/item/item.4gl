...
#+ Method: getRecords
#+
#+ Get the item records from the database
#+
...
FUNCTION getRecords()
    ...
    ELSE LET sqlStatement = SFMT("%1 %2 '%3'", _SELECTSQL, " AND itemid LIKE ", mQueryID) END IF
...
#+
#+ Format the item records from using JSON encoding
#+
...
#+ Method: init()
#+
#+ Initialize the item record storage
#+
...
#+
#+ Accessor for the item records array.
#+
...
PRIVATE FUNCTION updateRecordById(thisData STRING)
    ...
    EXECUTE recordUpdate USING thisRecord.unitcost, thisRecord.itemid -- a list from the record thisDat.itemid
...
PRIVATE FUNCTION deleteRecordById(thisData STRING)
    ...
    EXECUTE recordDelete USING thisRecord.itemid
    LET stat = SQLCA.SQLCODE
