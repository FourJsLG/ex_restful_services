...
#+ Method: getRecords
#+
#+ Get the account records from the database
#+
...
FUNCTION getRecords()
    ...
    ELSE LET sqlStatement = SFMT("%1 %2 '%3'", sqlStatement, " AND userid LIKE ", mQuery.id) END IF
    
    IF ( mQuery.fname IS NULL ) THEN
    ELSE LET sqlStatement = SFMT("%1 %2 '%3'", sqlStatement, " AND firstname LIKE ", mQuery.fname) END IF

    IF ( mQuery.lname IS NULL ) THEN
    ELSE LET sqlStatement = SFMT("%1 %2 '%3'", sqlStatement, " AND lastname LIKE ", mQuery.lname) END IF
...
#+
#+ Accessor for the account records array.
#+
...
#+ @return stat INTEGER : HTTP status code
#+
FUNCTION processRecordsDelete(deletePayload STRING)
    DEFINE thisJSONArr  util.JSONArray
    DEFINE thisJSONObj  util.JSONObject
    DEFINE i INTEGER 
