...
#
################################################################################################################################################################
#+ This module implements orderItem class information handling
#+
#+ This code uses the 'officestore' database tables.
#+ orderItem input, query and list handling functions are defined here.
#+

IMPORT util
SCHEMA officestore

&include "orderItemTypes.inc"
...
#+ Method: getRecords
#+
#+ Get the orderItem records from the database
#+
...
FUNCTION getRecords()
    ...
    IF ( mQuery.id IS NULL ) THEN
    ELSE LET sqlStatement = SFMT("%1 %2 '%3'", sqlStatement, " AND orderid LIKE ", mQuery.id) END IF
...
#+
#+ Format the orderItem records from using JSON encoding
#+
...
#+ Method: init()
#+
#+ Initialize the orderItem record storage
#+
...
#+
#+ Accessor for the orderItem records array.
#+
...
#+
#+ Update the orderItem records in the data source.
#+
#+    Perform the tasks to update the changes to the sample records for storage
#+    to the data source
#+
#+ @code
...
#+
FUNCTION processRecordsUpdate(thisData STRING)
...
#+
#+ Update the given orderItem record in the data source.
#+
#+    Executes the update to the given sample record in the datasource returning  
...
#+
PRIVATE FUNCTION updateRecordById(thisData STRING)
    ...
    --    PREPARE recordUpdate FROM _UPDATESQL
    EXECUTE recordUpdate USING thisRecord.unitprice,   -- a list from the record thisData.orderid and linenum
                               thisRecord.orderid,
                               thisRecord.linenum    
...
#+
#+ Performs tasks to insert orderItem record information into the datasource.
#+
#+    Executes the insert into the datasource returning success/fail 
#+
#+ @code
...
#+
FUNCTION processRecordsInsert(thisData STRING)
    DEFINE thisJSONArr  util.JSONArray
    DEFINE thisJSONObj  util.JSONObject
    DEFINE i INTEGER 
...
#+
#+ Creates the given orderItem record in the data source.
#+
#+    Executes the insert of the given sample record into the datasource returning  
#+    the number of rows processed.  If no rows found, return zero.
#+
#+ @code
...
#+
#+ Performs tasks to delete the orderItem records from the datasource.
#+
...
#+ @return stat INTEGER : HTTP status code
#+
FUNCTION processRecordsDelete(thisData STRING)
    DEFINE thisJSONArr  util.JSONArray
    DEFINE thisJSONObj  util.JSONObject
    DEFINE i INTEGER 
...
#+
#+ Delete the given orderItem record in the data source.
#+
#+    Executes the update to the given sample record in the datasource returning  
...
--    PREPARE recordDelete FROM _DELETESQL
    EXECUTE recordDelete USING thisRecord.orderid, thisRecord.linenum
