################################################################################
#+ This module implements product class information handling
#+
#+ This code uses the 'officestore' database tables.
#+ Product input, query and list handling functions are defined here.
#+

IMPORT util
SCHEMA officestore

&include "productTypes.inc"

#
# Module variables
#
DEFINE mQueryID STRING 
...
FUNCTION getRecords()
    ...
    IF ( mQueryId IS NULL ) THEN LET sqlStatement = _SELECTSQL
    ELSE LET sqlStatement = SFMT("%1 %2 '%3'", _SELECTSQL, " AND productid LIKE ", mQueryID) END IF
...
#+
FUNCTION updateRecordById(thisData STRING)
    ...
    ##For brevity in demo, only updating product name and description...it could be all fields
    EXECUTE recordUpdate USING thisRecord.prodname, thisRecord.proddesc, thisRecord.productid -- a list from the record thisAccount.firstname, 
...
#+
# Description: Performs tasks to insert information
#
# Return: HTTP Status
#
...
FUNCTION insertRecord(thisData)
...
#+ @return NONE
#+
FUNCTION deleteRecordById(thisData STRING)
    DEFINE thisRecord  recordType
    DEFINE parseObject util.JSONObject
    DEFINE stat INT
    ...
    EXECUTE recordDelete USING thisRecord.productid
    LET stat = SQLCA.SQLCODE
